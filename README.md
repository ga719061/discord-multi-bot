...
- **細分監控開關**：使用 `/設定紀錄` 喚出**圖形化控制面板**，自由勾選您感興趣的紀錄類別。
...
### 🛒 皇家採購 & 實用
- **Steam 指令**：`/特價查詢 搜尋` (搜尋價格), `/設定特價推播 特價列表` (手動投放特價榜單), `/設定特價推播 設定頻道` (每日特價推播設定)。
- **遊戲戰績**：`/戰績 特戰英豪` 讀取 ValoCheck 當前 Act 的 **All Modes** 輸出、命中與特務表現；`/戰績 英雄聯盟` 讀取 OP.GG 的當前賽季 KDA、勝率與常用英雄表現，不需 API Key。
- **智能提醒**：`/提醒 設定` 定時提醒功能。
- **自定義歡迎**：`/設定歡迎` 溫馨入城饗宴。
...
### 🔄 指令同步
- 正式部署請勿設定 `GUILD_ID`：啟動時會同步全域指令，並清除各伺服器殘留的舊指令，例如 `/volume`、`/stop`、`/shuffle`。
- 只在開發預覽時設定 `GUILD_ID`：同步只會作用於該測試伺服器，不會發布或清除全域指令。
