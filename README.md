...
### 🛒 皇家採購 & 實用
- **Steam 指令**：使用 `/特價查詢` 開啟皇家採購彈窗並輸入遊戲名稱；結果先私下呈報，可由查詢者單次頒布到原頻道。Administrator 可在 `/設定` 面板管理每日推播與確認後手動投放特價榜單。
- **遊戲戰績**：使用 `/戰績` 進入皇家戰報廳，選擇特戰英豪或英雄聯盟後於彈窗填入 Riot ID；結果先私下呈報，可單次頒布到原頻道。VALORANT 優先讀取 OP.GG 當前 Act 的 **All Modes** 公開輸出、命中、特務、武器與地圖表現，OP.GG 未公開資料時會嘗試 ValoCheck 備援；英雄聯盟讀取 OP.GG 的當前賽季 KDA、勝率與常用英雄表現。全程不需 API Key，結果卡片會標示實際來源並提供連結。
- **智能提醒**：`/提醒 設定` 定時提醒功能。
