...
- **集中管理面板**：Administrator 可從 `/幫助` 首頁進入管理控制台，或直接使用 `/設定`，先查看完整健康總覽，再管理歡迎、日誌、等級、Steam、身分組、AI、公告、系統狀態與成員查詢。
...
### 🛒 皇家採購 & 實用
- **Steam 指令**：`/特價查詢 搜尋` 搜尋價格；Administrator 可在 `/設定` 面板管理每日推播與確認後手動投放特價榜單。
- **遊戲戰績**：`/戰績 特戰英豪` 優先讀取 OP.GG VALORANT 當前 Act 的 **All Modes** 公開輸出、命中、特務、武器與地圖表現，OP.GG 未公開資料時會嘗試 ValoCheck 備援；`/戰績 英雄聯盟` 讀取 OP.GG 的當前賽季 KDA、勝率與常用英雄表現。全程不需 API Key，結果卡片會標示實際來源並提供連結。
- **智能提醒**：`/提醒 設定` 定時提醒功能。
