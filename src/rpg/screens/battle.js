...
export async function renderBattle(interaction, battleId, actionLog = '') {
    ...
    ).setFooter({ text: `🐕👑 吉吉王國冒險者公會 | 頻道: ${interaction.channel?.name || '未知'}` });
...
export async function handleBattleAction(interaction) {
    ...
    if (!battle.player_ids.includes(userId)) {
        return interaction.reply({ content: '🐕 你不在這場戰鬥中喔！', flags: ['Ephemeral'] });
    }
    const ps = battle.player_states[userId];
    battle.ally_summons = battle.ally_summons || [];
    if (ps.hp <= 0 && !id.startsWith('rpg_battle_flee_')) {
        return interaction.reply({ content: '🐕 你已經倒下了，無法行動！', flags: ['Ephemeral'] });
    }
    ...
    try {
        ...
        if (hasState(ps, 'stunned')) {
            ...
            if (id.startsWith('rpg_target_select_') && interaction.isStringSelectMenu()) {
                ...
                if (actionType === 'SKILL_ALLY') {
                    ...
                    if (!skill) {
                        ...
                        return interaction.reply({ content: '🐕 找不到技能資料！', flags: ['Ephemeral'] });
                    }
                    ...
                    if (!target) {
                        ...
                        return interaction.reply({ content: '🐕 找不到目標！', flags: ['Ephemeral'] });
                    }
                    ...
                    if (!target) {
                        ...
                        return interaction.reply({ content: '🐕 找不到目標魔物！', flags: ['Ephemeral'] });
                    }
                    if (target.currentHp <= 0) {
                        return interaction.reply({ content: '🐕 目標已經被打倒了，請重新選擇！', flags: ['Ephemeral'] });
                    }
            ...
            // ===== 技能選擇 (按鈕) =====
                ...
                if (skills.length === 0) {
                    return interaction.reply({ content: '🐕 你目前沒有可用的上場技能！請在角色資訊中設定。', flags: ['Ephemeral'] });
                }
                ...
                if (skill.type === 'heal' || skill.type === 'buff' || skill.type === 'shield' || skill.target === 'party') {
                    ...
                    if (skill.special === 'life_guard' || skill.special === 'full_revive_guard') {
                        ...
                        if (aliveSummons >= maxSummons) {
                            return interaction.reply({ content: `🐕 場上召喚物已達上限 (${maxSummons}隻)！`, flags: ['Ephemeral'] });
                        }
            ...
            // ===== 逃跑 (按鈕) =====
                ...
                if (Math.random() * 100 < escapeChance) {
                    ...
                    const payload = { embeds: [rpgEmbed('🏃 逃跑成功！', `<@${userId}> 成功逃離了戰鬥！汪！`)], components: [backButton()] };
                    if (interaction.deferred || interaction.replied) {
                        return interaction.editReply(payload);
                    }
                    return interaction.update(payload);
