...
export const SKILLS = {
    warrior: [
        ...
        { id: 'iron_skin', name: '鋼鐵護體', emoji: '🛡️', mp: 12, type: 'buff', target: 'self', effect: { defPercent: 40, mdefPercent: 20, turns: 3 }, cd: 3, desc: '自身 DEF +40%, MDEF +20% (3回合) (CD: 3)' },
        { id: 'sweep', name: '衝擊波', emoji: '🧹', mp: 15, type: 'physical', target: 'all', multiplier: 1.2, debuff: { stat: 'spd', percent: -30, turns: 2 }, cd: 2, desc: 'ATK×1.2 全體傷害 + 減速 30% (CD: 2)' },
        { id: 'whirlwind', name: '旋風斬', emoji: '🌪️', mp: 18, type: 'physical', target: 'all', multiplier: 1.3, hits: 2, cd: 1, desc: 'ATK×1.3 旋風兩連擊 (CD: 1)' },
        { id: 'vanguard_charge', name: '衝鋒', emoji: '🐗', mp: 20, type: 'physical', target: 'single', multiplier: 2.0, stunChance: 40, effect: { taunt: true, turns: 2 }, cd: 2, desc: 'ATK×2.0 衝擊 + 40% 眩暈 + 嘲諷 (CD: 2)' },
    ...
    ranger: [
        ...
        { id: 'rapid_fire', name: '三重矢', emoji: '⚡', mp: 25, type: 'physical', target: 'single', hits: 3, multiplier: 1.1, cd: 1, desc: 'ATK×1.1 快速三連射 (CD: 1)' },
        { id: 'toxic_arrow', name: '劇毒之箭', emoji: '☠️', mp: 12, type: 'physical', target: 'single', multiplier: 1.0, dot: { type: 'poison', percent: 10, turns: 5 }, cd: 2, desc: 'ATK×1.0 + 長效中毒 (5回合) (CD: 2)' },
    ...
    mage: [
        { id: 'fireball', name: '火球', emoji: '🔥', mp: 12, type: 'magical', target: 'single', multiplier: 1.4, dot: { type: 'burn', percent: 12, turns: 3 }, desc: 'MATK×1.4 + 持續三回合燃燒' },
        { id: 'mana_surge', name: '魔力增幅', emoji: '✨', mp: 0, type: 'buff', target: 'self', recoverMp: 50, effect: { matkPercent: 50, turns: 2 }, cd: 2, desc: '回復 50 MP 並大幅提升魔攻 (CD: 2)' },
        { id: 'frost_nova', name: '極道落雷', emoji: '⚡', mp: 20, type: 'magical', target: 'all', multiplier: 1.5, stunChance: 45, desc: '全體強效雷擊並有機率造成眩暈' },
        { id: 'summon_contract', name: '召喚術', emoji: '📜', mp: 30, type: 'summon', special: 'mage_summon', cd: 3, desc: '根據施法者等級召喚魔物助戰 (CD: 3)' },
    ...
    paladin: [
        { id: 'holy_strike', name: '聖威一擊', emoji: '✨', mp: 10, type: 'mixed', target: 'single', multiplier: 1.2, matkMultiplier: 1.2, stunChance: 25, undeadBonus: 2.0, desc: '神聖混傷打擊，高機率眩暈 (CD: 1)', cd: 1 },
        { id: 'divine_heal', name: '神聖治癒', emoji: '💚', mp: 20, type: 'heal', target: 'single_ally', healPercent: 35, effect: { cleanse: true }, cd: 2, desc: '恢復 35% HP 並解除負面狀態 (CD: 2)' },
        { id: 'guardian_vow', name: '守護誓約', emoji: '🛡️', mp: 30, type: 'buff', target: 'party', effect: { defPercent: 30, mdefPercent: 30, turns: 4 }, cd: 3, desc: '全隊防禦大幅提升 (4回合) (CD: 3)' },
        { id: 'retribution', name: '審判之盾', emoji: '⚖️', mp: 25, type: 'buff', target: 'self', effect: { reflect: 50, turns: 3 }, cd: 4, desc: '反彈 50% 物理傷害 (3回合) (CD: 4)' },
        { id: 'divine_barrier', name: '聖光加持', emoji: '🕊️', mp: 40, type: 'buff', target: 'party', effect: { debuffImmunity: 2, reduction: 20, turns: 2 }, cd: 5, desc: '全隊免疫負面並減傷 (CD: 5)' },
        { id: 'life_guard', name: '聖光救贖', emoji: '🌟', mp: 60, type: 'special', target: 'party', special: 'life_guard', cd: 8, desc: '全隊復活或生命保底 (CD: 8)' },
        { id: 'consecration', name: '靈魂昇華', emoji: '⛪', mp: 18, type: 'buff', target: 'party', effect: { hpRegen: 8, mpRegen: 5, turns: 3 }, cd: 3, desc: '全隊持續回血回魔 (3回合) (CD: 3)' },
        { id: 'condemnation', name: '定罪', emoji: '⚖️', mp: 22, type: 'mixed', target: 'single', multiplier: 2.2, matkMultiplier: 1.5, healPercent: 15, cd: 2, desc: '懲戒打擊並轉化為群體治療 (CD: 2)' },
        { id: 'absolute_guard', name: '不壞金身', emoji: '🛡️', mp: 50, type: 'buff', target: 'single_ally', effect: { invulnerable: 1 }, cd: 5, desc: '使隊友進入 1 回合無敵 (CD: 5)' },
        { id: 'divine_field', name: '至高神域', emoji: '🕊️', mp: 85, type: 'buff', target: 'party', effect: { shieldMultiplier: 6.0, hpRegen: 20, turns: 3 }, cd: 6, desc: '構築極限護盾與重生 (CD: 6)' },
        { id: 'divine_dawn', name: '究極聖療', emoji: '☀️', mp: 120, type: 'special', target: 'party', special: 'full_revive_guard', cd: 10, desc: '全隊完全復活並獲得神之守護 (CD: 10)' },
    ],
    magic_swordsman: [
        ...
        { id: 'frost_slash', name: '極寒冰裂', emoji: '❄️', mp: 22, type: 'mixed', target: 'single', multiplier: 2.0, matkMultiplier: 2.0, debuff: { stat: 'spd', percent: -40, turns: 2 }, cd: 1, desc: '凍結敵人血液的混傷重擊 (CD: 1)' },
        { id: 'thunder_strike', name: '雷霆劍氣', emoji: '⚡', mp: 25, type: 'mixed', target: 'single', multiplier: 2.5, matkMultiplier: 2.5, stunChance: 50, cd: 2, desc: '雷鳴強效混傷與長效眩暈 (CD: 2)' },
        { id: 'elemental_burst', name: '元素迸發', emoji: '🌈', mp: 45, type: 'magical', target: 'all', multiplier: 2.0, effect: { randomStatus: ['burn', 'poison', 'freeze'] }, cd: 3, desc: '混沌元素爆裂，隨機施加狀態 (CD: 3)' },
        ...
        { id: 'void_crack', name: '時空裂痕', emoji: '🕳️', mp: 20, type: 'magical', target: 'all', multiplier: 1.8, ignoreDef: true, cd: 1, desc: '撕裂空間的絕對魔力傷害 (CD: 1)' },
        { id: 'elemental_overload', name: '元素超載', emoji: '💥', mp: 30, type: 'buff', target: 'self', effect: { atkPercent: 60, matkPercent: 60, spdPercent: 60, turns: 2 }, cd: 5, desc: '兩回合的極限超載狀態 (CD: 5)' },
        { id: 'elem_enchant', name: '全元素附體', emoji: '🌈', mp: 40, type: 'buff', target: 'self', effect: { randomEnchant: true, turns: 5 }, cd: 4, desc: '獲得隨機屬性的強效附魔 (CD: 4)' },
        { id: 'mana_storm', name: '魔力風暴', emoji: '🌀', mp: 90, type: 'mixed', target: 'all', multiplier: 3.5, matkMultiplier: 3.5, cd: 5, desc: '毀滅性的元素魔力風暴 (CD: 5)' },
